...
Budgeting never reorders hits or changes which rows recall selected. In order, it drops the redundant `snippet` field, then drops a suffix of the already-ranked list, and only when a single hit cannot fit whole does it truncate that one hit's `content` — always marked, inline and via `content_truncated`/`content_omitted_chars`. A hit's `explanation` scaffolding is never shortened, so a hit whose fixed overhead alone busts the cap is dropped rather than shipped over budget wearing a marker that saved nothing. That is the one case where a nonempty matching corpus can return zero `hits`; the response says so (`hits_available > 0`, `hits_returned: 0`) rather than quietly overshooting.

A `recall_budget` object (`hits_available`, `hits_returned`, `hits_dropped`, `snippet_dropped`, `content_truncated_memory_ids`, `applied`) is always present, even on a no-op, so a caller asking for `k=100` and getting fewer can tell the budget did it. The same budget covers MCP `memory_recall` and `memory_timeline`, `haunt recall --json`, and both dashboard recall endpoints — the all-namespace fan-out is budgeted once, not per namespace, and its `namespace_groups` always agree with the bounded `hits`. The `[haunt ns=...]` block the hooks inject has its own smaller `HAUNT_RECALL_BLOCK_MAX_CHARS` with the same drop-the-tail, never-silent behavior on every return path, including the empty result and a namespace too long to fit the cap at all. Human CLI table output is unaffected — its lines are already snippet-bounded.
