...
- **Hooks store automatically.** Once `haunt install` runs, prompts and responses are stored verbatim in Cursor and Claude Code. Tool input/output is best-effort redacted, capped per field, and can be dropped from capture entirely per tool (`HAUNT_EXCLUDE_TOOLS`, see Environment variables). New writes also carry a versioned source-provenance envelope; hook writes record their actual channel and supplied tool/call IDs.
...
The console binds **127.0.0.1** and mints a random launch token that every `/api` route requires. On loopback the page injects the token for you; `--allow-remote` does not, and is **unsafe without it** — it exposes the memory admin API on the network. Namespaces are storage isolation, not authorization. [SECURITY.md](SECURITY.md#dashboard-bind-host-origin-and-launch-token) has the full bind, `Host`, `Origin`, and token rules.
...
Auto-store prompts and replies verbatim, plus best-effort-redacted and capped tool I/O. No LLM and no summaries. Fail-open (`{}` + exit 0) so a hook *error* never blocks the agent — see [SECURITY.md](SECURITY.md#fail-open-hooks) for what fail-open does not cover.
...
Retrieval responses are size-budgeted by `HAUNT_RECALL_MAX_CHARS` (see Environment variables): the `hits` list as actually serialized — brackets and separators included — never exceeds the cap. The surrounding envelope (`namespace`, `query`, `trust_policy`, `execution`) is not counted; it does not grow with the corpus.

Budgeting never reorders hits or changes which rows recall selected. In order, it drops the redundant `snippet` field, then drops a suffix of the already-ranked list, and only when a single row cannot fit whole does it truncate that one row's text — always marked, inline and via `<field>_truncated`/`<field>_omitted_chars`. A recall hit's text is `content`; an event row's is `tool_output` then `tool_input` then `content`, cut in that order, because a hook-captured tool row carries its bulk there and an empty `content`. A hit's `explanation` scaffolding is never shortened, so a hit whose fixed overhead alone busts the cap is dropped rather than shipped over budget wearing a marker that saved nothing. That is the one case where a nonempty matching corpus can return zero `hits`; the response says so (`hits_available > 0`, `hits_returned: 0`) rather than quietly overshooting.

A `recall_budget` object (`hits_available`, `hits_returned`, `hits_dropped`, `snippet_dropped`, `content_truncated_memory_ids`, `applied`) is always present, even on a no-op, so a caller asking for `k=100` and getting fewer can tell the budget did it. The same budget covers MCP `memory_recall` and `memory_timeline`, `haunt recall --json`, and both dashboard recall endpoints — the all-namespace fan-out is budgeted once over the namespaces merged round-robin by local rank, not per namespace and not in collation order, so no namespace loses its top hit while another ships its seventh; its `namespace_groups` always agree with the bounded `hits` and each carries its own `hits_available`/`hits_dropped`. `memory_timeline` orders events newest-first, so a budgeted window loses its oldest rows and says so in `window_truncated`. The `[haunt ns=...]` block the hooks inject has its own smaller `HAUNT_RECALL_BLOCK_MAX_CHARS` with the same drop-the-tail, never-silent behavior on every return path, including the empty result and a namespace too long to fit the cap at all. Human CLI table output is unaffected — its lines are already snippet-bounded.
