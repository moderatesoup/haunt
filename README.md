...
| `afterAgentThought` | skipped by default (`HAUNT_STORE_THOUGHTS=1` to store as system/coordinate, captured but not embedded) |
...
Retrieval responses are additionally size-budgeted (`HAUNT_RECALL_MAX_CHARS`, see Environment variables): the serialized `hits` list is bounded before crossing into agent context, and this bound is absolute — the list as actually serialized, brackets and separators included, never exceeds the configured cap, regardless of where a hit's bulk comes from. The cap covers the `hits` list, not the surrounding response envelope (`namespace`, `query`, `trust_policy`, `execution`), which is small and does not grow with corpus size. Binding only ever drops the redundant `snippet` field, drops a suffix of the already-ranked `hits` list, or — only when a single hit alone cannot fit whole — truncates that one hit's `content` with an explicit inline marker plus `content_truncated`/`content_omitted_chars`; it never reorders hits or changes which rows recall selected. Content truncation only happens when it actually helps: a hit's non-content scaffolding (its `explanation`, e.g. an unbounded `correction_lineage.correction_ids` list or a multi-KB validated `provenance` envelope) is never shortened, so if that fixed overhead alone already exceeds the budget, cutting `content` wouldn't help either — the hit is dropped instead of being shipped over budget wearing a truncation marker that saved nothing. That is the one case where recall can return fewer hits than exist: a nonempty, matching corpus can come back with zero `hits` (`hits_available > 0`, `hits_returned: 0`) if even the top-ranked hit's fixed overhead alone exceeds the cap — a deliberate exception to "a nonempty result never returns zero hits", chosen because `recall_budget` describing truthfully what crossed the boundary matters more than always returning something. A `recall_budget` object (`hits_available`, `hits_returned`, `hits_dropped`, `snippet_dropped`, `content_truncated_memory_ids`, `applied`) is always present on the response, even when the budget was a no-op, so a caller asking for `k=100` and getting fewer can tell the budget did it; `recall_budget` never reports `applied: True` with `hits_dropped: 0` while the response is actually over budget. The `[haunt ns=...]` block both hooks inject as automatic per-prompt context has its own, smaller budget (`HAUNT_RECALL_BLOCK_MAX_CHARS`) with the same drop-the-tail, never-silent behavior, and the cap is enforced on every return path, not just the common one: the trailing `"...N more hit(s) omitted..."` marker's own cost is reserved before packing lines so it can never itself push the block over cap, an empty result still respects the cap rather than returning `(no memories)` unchecked, and even the degenerate case of a namespace too long to fit the cap at all (both real hooks clamp it to 80 chars first, so this is the function's own contract, not a live path) truncates the header itself with its own inline marker instead of silently slicing through it — so the returned block never itself exceeds the configured cap, and whenever anything is left out, whether a hit line or the namespace text, the block always says so. The same `hits` budget and the same `recall_budget` object apply to MCP `memory_timeline` (over `events`), `haunt recall --json`, and both dashboard recall endpoints; the dashboard's all-namespace endpoint budgets the whole fan-out once rather than per namespace, and its `namespace_groups` always agree with the bounded `hits`. Human CLI table output is unaffected — its lines are already snippet-bounded.
...
| `HAUNT_RERANK_ENABLED` | unset | set to `1` to enable the deterministic lexical MMR diversity rerank (`haunt.rerank`) inside `recall()`, so CLI, MCP, hooks and dashboard all rerank together. **Off by default**; on, recall fuses a wider candidate pool, trims it to `k` by MMR, and each hit's `explanation.ordering` reports that stage with the fusion rank it moved from in `explanation.rrf_rank` |
| `HAUNT_RERANK_LAMBDA` | `0.3` | relevance/diversity tradeoff for that reranker (clamped 0–1; `1.0` relevance only, `0.0` diversity only). Only read when `HAUNT_RERANK_ENABLED` is on |
