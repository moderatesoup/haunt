...
Purge also sweeps the namespace backups haunt writes itself. `namespaces reconcile --apply` and `namespaces retire --apply` each leave a full plaintext copy of a database under `HAUNT_HOME/backups`, so the erasure was true in the live file and false one directory away. Purge now erases the row from every such backup that holds it and rebuilds each one it touches, matching on the memory id reconcile preserves when it copies rows; `backups_erased` and `backups_unerased` report the outcome, and CLI, MCP, and console all surface a backup that still holds the text. Sweeping invalidates the sha256 the migration report recorded for that backup, and a swept backup restores a database whose erased content is gone but whose identifiers were never rekeyed. Copies haunt did not write — exports, an operator's own copy, filesystem snapshots, removable media — and blocks the filesystem already released remain out of reach; SECURITY.md states the boundary.
