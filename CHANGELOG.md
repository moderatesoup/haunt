...
Purge also sweeps the namespace backups haunt writes itself. `namespaces reconcile --apply` and `namespaces retire --apply` each leave a full plaintext copy of a database under `HAUNT_HOME/backups`, so the erasure was true in the live file and false one directory away. Purge now runs its whole erasure on every such backup that holds the row — content, correction lineage, purge-safe session rekeying and metadata sanitization, and the privacy head rotation — and rebuilds each file it touches, matching on the memory id reconcile preserves when it copies rows. A restored backup is therefore a database the purge has already happened to: it refuses a bundle exported before the purge instead of importing the erased row back. A file counts as erased only after the sweep re-reads it — integrity and foreign-key checks, the row's absence, a moved privacy head, and a raw byte scan for the erased values — so `backups_unerased` names anything unproven, and CLI, MCP, and console all surface it. Sweeping invalidates the sha256 the migration report recorded for that backup. Copies haunt did not write — exports, an operator's own copy, filesystem snapshots, removable media — and blocks the filesystem already released remain out of reach; SECURITY.md states the boundary.
