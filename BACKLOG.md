...
| L9 | Embedding ingest wastes throughput on padding: `tokenizers` pads each batch to its longest member, so a randomly ordered batch of 100 turns is almost always padded to 512, roughly 2.6x the tokens actually needed. Length-sorting before batching measured 3.34x (3.14 to 10.49 texts/s at batch=16) with vectors unchanged (min cosine 1.00000000, max absolute difference 6.6e-07). This affects every user's ingest, not only the benchmark. FIXED: length-sorted sub-batching shipped, measured 1.98x through `embed_texts()` with vectors identical at min cosine 1.0000000000. | MEDIUM (performance, all users) | `perf/embed-batching` `6198083` | operator |
...
|---|---|---|
| L17: reconcile refused a fresh-vs-migrated pairing on column ORDER | FIXED | `_reconcile_content_state_digest` compared `PRAGMA table_info` as an ordered list, so `events.idempotency_key` at ordinal 12 (migrated) vs 1 (fresh) refused a merge whose column sets were identical -- the exact pairing reconcile exists to heal, reproduced on both real split pairs. Now compares a name-keyed signature of (type, notnull, default, pk): strictly stronger, since the old check saw names only and was blind to NOT NULL. Verified nothing downstream needs ordering. CHECK/FK/index differences remain uncompared and are documented as such. |
| R15: `execution` computed from `hits` rather than `bounded_hits` | NOT A DEFECT | `execution_metadata` reads `getattr(hits, "execution")` on the planner's result object, not per-hit data; a plain list carries none, so passing `bounded_hits` would return `None` and drop the block entirely. Rationale now inline at `src/haunt/mcp_server.py`. |
